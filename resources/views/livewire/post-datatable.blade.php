            ...
            <table class="w-full whitespace-no-wrap data-table dark:text-gray-200" style="color:unset; width:100%">
                <thead>
                    <tr
                        ...
                        <th wire:click="sortBy('posts.id')" class="px-4 py-3 cursor-pointer">Id <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" width="15px" height="15px" style="display: inline;" fill="currentColor">
                            <path fill-rule="evenodd" d="M10 3a1 1 0 01.707.293l3 3a1 1 0 01-1.414 1.414L10 5.414 7.707 7.707a1 1 0 01-1.414-1.414l3-3A1 1 0 0110 3zm-3.707 9.293a1 1 0 011.414 0L10 14.586l2.293-2.293a1 1 0 011.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clip-rule="evenodd" />
                          </svg></th>
                          <th wire:click="sortBy('users.first_name')" class="px-4 py-3 cursor-pointer">Post By <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" width="15px" height="15px" style="display: inline;" fill="currentColor">
                            <path fill-rule="evenodd" d="M10 3a1 1 0 01.707.293l3 3a1 1 0 01-1.414 1.414L10 5.414 7.707 7.707a1 1 0 01-1.414-1.414l3-3A1 1 0 0110 3zm-3.707 9.293a1 1 0 011.414 0L10 14.586l2.293-2.293a1 1 0 011.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clip-rule="evenodd" />
                          </svg></th>
                          <th wire:click="" class="px-4 py-3 cursor-pointer">Message </th>
                          <th wire:click="" class="px-4 py-3 cursor-pointer">Likes </th>
                          <th wire:click="" class="px-4 py-3 cursor-pointer">Dislikes </th>
                          <th wire:click="" class="px-4 py-3 cursor-pointer">Comments </th>
                ...
                <tbody class="bg-transperent divide-y dark:divide-gray-700">
                    @foreach ($posts as $post)
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                            ...
                            <td class="px-4 py-3"> {{ $post->post_by }} </td>
                            <td class="px-4 py-3"><textarea disabled width="100" height="150" > {{ $post->message }}</textarea> </td>
                            <td class="px-10 py-10"> {{ $post->total_likes }} </td>
                            <td class="px-10 py-10"> {{ $post->total_unlikes }} </td>
                            <td class="px-10 py-10"> {{ $post->total_comments }} </td>
                            <td>                                        
                                <div class="flex space-x-1">&nbsp;&nbsp;
                                    <a href="{{route('post.detail',$post->id)}}" class="p-1 text-blue-600 hover:bg-blue-600 hover:text-white rounded">
                                    ...
                                    <button wire:click="confirmDelete({{ $post->id }})" class="deletePost p-1 text-red-600 hover:bg-red-600 hover:text-white rounded">
            ...
            <div class=" dark:bg-gray-800  dark:text-gray-200 rounded-lg rounded-t-none max-w-screen rounded-lg border-b border-gray-200 bg-white">
              <div class="dark:bg-gray-800 p-2 sm:flex items-center justify-between">
                <div class="dark:bg-gray-800 my-2 sm:my-0 flex items-center">
                  <select wire:model="perPage" id="perPage" class="per-page dark:bg-gray-800 mt-1 form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                ...
                <div class="dark:bg-gray-800 my-4 sm:my-0">
                  <div  class="dark:bg-gray-800 lg:flex justify-center">
                      <span class="dark:bg-gray-800">{{ $posts->links('livewire.datatables.tailwind-pagination') }}</span>
                  </div>
                </div>

                <div class="flex justify-end text-gray-600 dark:bg-gray-800">
                  Posts {{ $posts->firstItem() }} - {{ $posts->lastItem() }} of
                  {{ $posts->total() }}
                </div>
              </div>
            </div>
          ...
          <div class="h-auto p-4 mx-2 text-left bg-white rounded shadow-xl md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="open = false">
            ...
            <div class="mt-5 sm:mt-6">
              <span class="flex rounded-md shadow-sm">
                ...
                <button wire:click="postdelete()" class="inline-flex ml-4 justify-center w-full px-4 py-2 text-white bg-red-500 rounded hover:bg-red-700">
                    Delete!
                </button>
              </span>
            </div>
...
<script type="text/javascript">
  $(document).ready(function(){
      ...
      var perPage = localStorage.getItem('perPagePost');
