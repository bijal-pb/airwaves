    ...
    <div class="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
        <div class="w-full overflow-x-auto">
                ...
                <tbody class="bg-transperent divide-y dark:divide-gray-700">
                    @foreach ($users as $user)
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                            ...
                            <td class="px-10 py-10"> {{ $user->status }} </td>
                            <td>
                                <div class="edit-dt flex space-x-1">&nbsp;&nbsp;
                                    <a href="{{route('users.edit',$user->id)}}" class="p-1 text-blue-600 hover:bg-blue-600 hover:text-white rounded">
            ...
            {{-- <div class="text-gray-600 dark:text-gray-300">
                <span>
                    Showing {{ $users->firstItem()}} to {{ $users->lastItem() }} out of {{ $users->total() }} users
                </span>
                <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                  {{ $users->links('livewire.datatables.tailwind-pagination') }}
                </span>
            </div> --}}
        </div>
    </div>
...
<script type="text/javascript">
  $(document).ready(function(){
      var page = localStorage.getItem('user');
      var url = $(location).attr('href');
      
      $(document.body ).click(function() {
            var url = window.location.href;
            var page = window.location.href.split('page=')[1];
            localStorage.setItem('user',page);
      });
      
      $(document).on('change','.per-page',function(event){
          var p = $(this).val();
          localStorage.setItem('perPageUser',p);
      });

      var perPage = localStorage.getItem('perPageUser');
